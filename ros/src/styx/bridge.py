...
class Bridge(object):
    def __init__(self, conf, server):
        ...
        self.callbacks = {
            ...
            '/stop_waypoints': self.callback_path
    ...
    def publish_traffic(self, data):
        ...
        yaw = [math.atan2(dy, dx) for dx, dy in zip(
            data['light_pos_dx'], data['light_pos_dy'])]
    ...
    def callback_steering(self, data):
        self.server('steer', data={'steering_angle': str(
            data.steering_wheel_angle_cmd)})
    ...
    def callback_path(self, data):
        ...
        self.server('drawline', data={
                    'next_x': x_values, 'next_y': y_values, 'next_z': z_values})
